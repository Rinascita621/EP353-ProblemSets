...
: AudioProcessor(BusesProperties()
    ...
    parameters(*this, nullptr, juce::Identifier("Parameters"),
               std::make_unique<juce::AudioParameterFloat>("Size", "Size", juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f),
               std::make_unique<juce::AudioParameterFloat>("Width", "Width", juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f),
               std::make_unique<juce::AudioParameterFloat>("Decay", "Decay", juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f),
               std::make_unique<juce::AudioParameterFloat>("DryWet", "Dry/Wet", juce::NormalisableRange<float>(0.0f, 1.0f), 0.5f)),
    FDNreverb(parameters, 4, 1.5f)

{
    // Initialize other parameters and attachments
    ...
    drywetParameter = parameters.getRawParameterValue("Dry/Wet");
}
...
void ReverbAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // Process the entire block using FDNreverb
    FDNreverb.process(audioBlock);
}
...
//ReverbAudioProcessor::myFDNReverb::myFDNReverb(juce::AudioProcessorValueTreeState& state, int numDelayLines, float initialDecay)
//    : numdelayLines(numDelayLines),
//      delayLines(numDelayLines, std::vector<float>(maxDelaylength, 0.0f)),
//      decay(initialDecay),
//      parameters(state)
//{
//    initializeMixmatrix();
//}
//
//void ReverbAudioProcessor::myFDNReverb::prepare(const juce::dsp::ProcessSpec& spec)
//{
//    // implementation preparation code
//}
//
//void ReverbAudioProcessor::myFDNReverb::process(juce::dsp::AudioBlock<float>& block)
//{
